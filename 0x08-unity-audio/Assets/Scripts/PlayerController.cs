...
public class PlayerController : MonoBehaviour
{
    ...
    public AudioSource sourceGrass;
    public AudioSource sourceRock;

    public AudioSource sourceLanding;
    ...
    void Update()
    {
        ...
        colliders = Physics.OverlapSphere(transform.position - sphereOffset, sphereRadius, ground);

        if(onGround)
        {
            foreach (Collider current in colliders)
            {
                GameObject parent = current.gameObject.transform.parent.parent.gameObject;
                groundType = parent.gameObject.CompareTag("GrassGround") == true ? "Grass" : "Rock";
        ...
        if (respawned == true && onGround && sourceLanding.isPlaying == false)
        {
            sourceLanding.Play();
            respawned = false;
        }
    ...
    void FixedUpdate()
    {
        ...
        if (direction.magnitude >= 0.1f)
        {
            ...
            if (groundType == "Grass" && onGround && sourceGrass.isPlaying == false)
            {
                sourceRock.Stop();
                sourceGrass.Play();
            }
            else if (groundType == "Rock" && onGround && sourceRock.isPlaying == false)
            {
                sourceGrass.Stop();
                sourceRock.Play();
            }
